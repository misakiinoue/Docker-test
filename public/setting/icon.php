...
//DBに接続
$dbh = new PDO('mysql:host=mysql;dbname=tech','root','');
//セッションにあるIDから、ログインしている対象の会員情報を引く
$select_sth = $dbh->prepare("SELECT * FROM users WHERE id = :id");
$select_sth->execute([
    'id' => $_SESSION['login__user_id'],
]);
$user = $select_sth->fetch();

if(isset($_POST['image_base64'])){
  //POSTで送られてくるフォームパラメータ image_base64がある場合

$image_filename = null;
if(!empty($_POST['image_base64'])){
  //先頭のbase:~base64のところは削る
  $base64 = preg_replace('/^data:.+base64,/', '', $_POST['image_base64']);

  //base64からバイナリにデコードする
  $image_binary = base64_decode($base64);

  //新しいファイル名を決めてバイナリに出力する
  $image_filename = strval(time()).bin2hex(random_bytes(25)). '.png';
  $filepath = '/var/www/upload/image/' . $image_filename;
file_put_contents($filepath,$image_binary);
}

//ログインしている会員情報のnameを更新する
